...
const NotesPanel = memo(function NotesPanel({
  ...
  return (
    <div className="bg-calendar-note rounded-lg p-4 sm:p-5 note-lines min-h-[200px] flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between mb-3 pb-2 border-b border-calendar-note-line">
      ...
      {range.start && (
        <motion.div
          ...
          className="text-xs font-sans text-muted-foreground mb-3 bg-calendar-range/50 px-3 py-1.5 rounded-md inline-flex gap-1 w-fit"
        >
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 mb-3">
        <input
          ...
          className="flex-1 px-3 py-2 rounded-md bg-background/60 border border-border text-sm font-sans text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
        />
        <button
          type="submit"
          disabled={!text.trim()}
          className="p-2 rounded-md bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-40 transition-all"
        >
          ...
          {relevantNotes.length === 0 ? (
            ...
            relevantNotes.map((note) => (
              <motion.div
                ...
                className="flex items-start gap-2 group py-1.5 px-2 rounded hover:bg-background/40 transition-colors"
              >
